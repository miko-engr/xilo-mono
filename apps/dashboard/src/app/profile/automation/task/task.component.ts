...
@Component({
  selector: 'app-automation-task',
  templateUrl: './task.component.html',
  styleUrls: ['../automation.component.css']
})


export class AutomationTaskComponent implements OnInit {
  ...
  ngOnInit() {
      this.templates$ = this.templateService.getByCompany('task');
      this.getCompany();
  }
  ...
  addTemplate(template: Template) {
      if (!this.template.title || !this.template.priority || !this.template.type || !this.template.description) {
        return this.logService.error('All fields are required');
      }
      template.isTask = true;
  ...
  deleteTemplate(template: Template) {
      if (confirm('Ae you sure you want to delete this task template?')) {
